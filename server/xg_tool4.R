...
output$boruta_plot <- renderPlotly({
  if (input$xgb_btn > 0) {
    ...
    if (!is.null(xgb_all)) {
      ...
      set.seed(42)
      boruta_result <- Boruta(as.formula(paste(target, "~ .")), data = train_data, doTrace = 2) # , getImp = getImpXgboost
      ...
      X_train_boruta <- train_data[, confirmed_attributes, drop = FALSE]
      X_test_boruta <- test_data[, confirmed_attributes, drop = FALSE]
      ...
      xgb.ggplot.shap.summary(as.matrix(X_test_boruta), contr, model = xgb_boruta_model)
