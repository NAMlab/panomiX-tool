...
observeEvent(input$dh_choosedataset, {
  ...
  if (dh_datasetSelected == "Upload-data") {
    if (is.null(dh_inFile1) || is.null(dh_inFile2) || is.null(dh_inFile3) || is.null(dh_meta)) {
      ...
      harmfile1 <- read.csv(dh_inFile1$datapath, row.names = 1)
      harmfile2 <- read.csv(dh_inFile2$datapath, row.names = 1)
      harmfile3 <- read.csv(dh_inFile3$datapath, row.names = 1)
      meta_data <- read.csv(dh_meta$datapath)
...
observeEvent(input$run, {
  ...
  if (!is.null(harm_all)) {
    ...
    tmp = t(as.data.frame(strsplit(sample_names, split=".", fixed=T)))
    scores$Sample <- sample_names
    scores$Treatment <- tmp[,1]
    scores$Stage <- substr(tmp[, 2], 1, 1)
    # combined list
    ...
    scores_T$Treatment <- tmp[,1]
    scores_T$Stage <- substr(tmp[, 2], 1, 1)
    # PCA1 list
    ...
    scores_P$Treatment <- tmp[,1]
    scores_P$Stage <- substr(tmp[, 2], 1, 1)
    # PCA2 list
    ...
    scores_M$Treatment <- tmp[,1]
    scores_M$Stage <- substr(tmp[, 2], 1, 1)
    # PCA3 list
