...
observeEvent(input$dh_choosedataset, {
  ...
  if (dh_datasetSelected == "Upload-data") {
    if (is.null(dh_inFile1) || is.null(dh_inFile2) || is.null(dh_meta)) {
      ...
      if (is.null(dh_inFile3)) {
        ...
        # Find common columns between harmfile1 and harmfile2
        common_columns <- intersect(colnames(harmfile1), colnames(harmfile2))
        
        # Subset data frames based on common columns
        ...
        # Pass the data to harm_data
        harm_data(list(harmfile1 = harmfile1, harmfile2 = harmfile2, meta = meta_data))
      } else {
        harmfile1 <- read.csv(dh_inFile1$datapath, row.names = 1, check.names = FALSE)
        harmfile2 <- read.csv(dh_inFile2$datapath, row.names = 1, check.names = FALSE)
...
observeEvent(input$run, {
  ...
  if (is.null(harm_all$harmfile3)) {
    ...
    # Concatenate
    combined_data <- rbind(df1_scaled_sqrt, df2_scaled_sqrt)
    ...
    datasets_r = c(rep("omics1", nrow(df1)), rep("omics2", nrow(df2)))
    datasets = c(rep("omics1", nrow(df1)), rep("omics2", nrow(df2)))
  ...
  if (!is.null(combined_pca_data_all)) {
    if (is.null(harm_all$harmfile3)) {
      ...
      ggplot(scores, aes(x = PC1, y = PC2, shape = Stage)) +
        ...
        labs(title = "Integrated multi-Omics PCA",
             x = paste("(PC1:", round(explained_variance_r[1] * 100, 2), "% ",
                       "omics1:", round(explained_variance_Z_r[2,1] * 100, 2), "% ",
                       "omics2:", round(explained_variance_Z_r[1,1] * 100, 2), "% ",
                       ")" ),
             y = paste("(PC2:", round(explained_variance_r[2] * 100, 2), "% ",
                       "omics1:", round(explained_variance_Z_r[2,2] * 100, 2), "% ",
                       "omics2:", round(explained_variance_Z_r[1,2] * 100, 2), "% ",
                       ")" )
             )
...
output$pca1_plot <- renderPlotly({
  ...
  if (!is.null(pca1_data_all) && !is.null(pca2_data_all)) {
    if (is.null(harm_all$harmfile3)) {
      ...
      subplot(list(p1, p2), nrows = 1, margin = 0.06, shareX = TRUE,
              titleY = TRUE)%>%  
        layout(annotations = list( 
          list(x = 0.13 , y = 1.05, text = "omics1", showarrow = F, xref='paper', yref='paper'),
          list(x = 0.5 , y = 1.05, text = "omics2", showarrow = F, xref='paper', yref='paper')) 
        )
    } else {
      scores_T <- pca1_data_all$scores_T
      pca_result1 <- pca1_data_all$pca_result1
      ...
      subplot(list(p1, p2, p3), nrows = 1, margin = 0.06, shareX = TRUE,
        ...
        layout(annotations = list( 
          list(x = 0.13 , y = 1.05, text = "omics1", showarrow = F, xref='paper', yref='paper'),
          list(x = 0.5 , y = 1.05, text = "omics2", showarrow = F, xref='paper', yref='paper'),
          list(x = 0.9 , y = 1.05, text = "omics3", showarrow = F, xref='paper', yref='paper')) 
        )
      }
    } else {
    output$error_notification <- renderUI({
...
output$pc_download <- renderUI({
  if (input$run > 0) {
    combined_pca_data_all <- combined_pca_data()
    if (!is.null(combined_pca_data_all)) {
      downloadButton("pc_data","Download_PCA")
    }}
})


output$pc_data <- downloadHandler(
  filename = function() {
    paste("Integrated_PCA_", Sys.Date(), ".svg", sep = "")
  },
  content = function(file) {
    ...
    if (!is.null(harm_all$harmfile3)) {
      ...
      p4 <- ggplot(scores, aes(x = PC1, y = PC2, shape = Stage)) +
        geom_point(size = 4) +
        geom_vline(xintercept = 0, linetype = "dashed", color = "grey") +
        geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
      ...
      p1 <- ggplot(scores_T, aes(x = PC1, y = PC2, shape = Stage)) +
        ...
        theme(legend.position = "none",
              ...
              plot.title = element_text(size = 9)) +
        labs(title = "Omics-1",
             x = paste("PC1 (", round(pca_result1$sdev[1]^2 / sum(pca_result1$sdev^2) * 100, 2), "%)", sep = ""),
             y = paste("PC2 (", round(pca_result1$sdev[2]^2 / sum(pca_result1$sdev^2) * 100, 2), "%)", sep = ""),
             color = "Treatment",
             shape = "Stage")
      ...
      p2 <- ggplot(scores_P, aes(x = PC1, y = PC2, shape = Stage)) +
        ...
        theme(legend.position = "none",
              ...
              plot.title = element_text(size = 9)) +
        labs(title = "Omics-2",
             x = paste("PC1 (", round(pca_result2$sdev[1]^2 / sum(pca_result2$sdev^2) * 100, 2), "%)", sep = ""),
             y = paste("PC2 (", round(pca_result2$sdev[2]^2 / sum(pca_result2$sdev^2) * 100, 2), "%)", sep = ""),
             color = "Treatment",
             shape = "Stage")
      ...
      p3 <- ggplot(scores_M, aes(x = PC1, y = PC2, shape = Stage)) +
        ...
        theme(legend.position = "none",
              ...
              plot.title = element_text(size = 9)) +
        labs(title = "Omics-3",
             x = paste("PC1 (", round(pca_result3$sdev[1]^2 / sum(pca_result3$sdev^2) * 100, 2), "%)", sep = ""),
             y = paste("PC2 (", round(pca_result3$sdev[2]^2 / sum(pca_result3$sdev^2) * 100, 2), "%)", sep = ""),
             color = "Treatment",
             shape = "Stage")
      
      # Arrange p2, p3, p4 horizontally in a single row
      small_plots_grob <- arrangeGrob(p1, p2, p3, nrow = 1, ncol = 3)
      
      # Combine the arranged grob (first row) with p1 (second row)
      PC_Integrated <- grid.arrange(
        small_plots_grob,
        p4,             
        nrow = 2,
        heights = c(0.35, 0.65)  # Adjust relative heights
      )
      
      # Save the final plot
      ggsave(filename = file, plot = PC_Integrated, device = "svg", width = 12, height = 8)
      } else {
        combined_pca_data_all <- combined_pca_data()
        pca1_data_all <- pca1_data()
        pca2_data_all <- pca2_data()
        ...
        # Arrange p2, p3, p4 horizontally in a single row
        small_plots_grob <- arrangeGrob(p1, p2, nrow = 1, ncol = 2)
